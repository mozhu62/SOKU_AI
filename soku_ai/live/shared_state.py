...
class SharedMemoryClient:
    ...
    def _validate_protocol(self) -> None:
        ...
        header = SharedState.from_address(self._address)
        expected_size = ctypes.sizeof(SharedState)
        if int(header.magic) != PROTOCOL_MAGIC:
            raise SharedMemoryProtocolError(
                f"共享内存 Magic 不一致: 0x{int(header.magic):08X}"
            )
        if int(header.protocolVersion) != PROTOCOL_VERSION:
            raise SharedMemoryProtocolError(
                "共享内存协议版本不一致: "
                f"dll={int(header.protocolVersion)}, python={PROTOCOL_VERSION}"
            )
    ...
    def read(self) -> SharedSnapshot | None:
        if not self.connect():
            return None
        assert self._address is not None
        sequence_address = self._address + SharedState.writeSequence.offset
        for _ in range(4):
            begin = ctypes.c_int32.from_address(sequence_address).value
            if begin & 1:
                continue
            raw = ctypes.string_at(self._address, ctypes.sizeof(SharedState))
            end = ctypes.c_int32.from_address(sequence_address).value
            if begin == end and not (end & 1):
                return SharedSnapshot(SharedState.from_buffer_copy(raw))
        return None
