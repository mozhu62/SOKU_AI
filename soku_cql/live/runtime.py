...
from .battle_state import role_error, mode_details
...
class LiveRuntime:
    ...
    def start(self):
        ...
        self.thread = threading.Thread(target=self._run, name="cql-live-evaluation", daemon=False)
        self.thread.start()
    ...
    def _role_error(self, p):
        return role_error(p, self.config["environment"])

    def _active_battle(self, p):
        return bool(p.inBattle and not self._role_error(p)
    ...
    def _observe(self, payload):
        old = self.latest
        self.latest = payload
        error = self._role_error(payload)
        if error:
    ...
    def _run(self):
        client = restart = None
        try:
            self.message = "加载 CQL 模型与 checkpoint 内归一化参数"
            ...
            self.message = "模型就绪。进入 1P 萃香 / CPU 灵梦后点击开始"
            ...
            while not self.closed.wait(env["poll_seconds"]):
                ...
                if serial == last_serial:
                    ...
                    continue
                last_serial = serial
                # 相同战斗帧可以发生场景/终局变化，但不能重复统计、反复更新 GRU。
