...
class Workbench:
    ...
    def __init__(self, config, auto_start=False):
        ...
        self.thread = threading.Thread(target=self._run, name="cql-workbench-coordinator", daemon=False)
    ...
    def snapshot(self):
        ...
        state.update(ui_mode="cql_live", workbench_busy=busy, workbench_error=error, runtime_config=config,
    ...
    def _execute(self, name, value):
        ...
        while not self.closed.wait(0.1):
            ...
            if state.get("loaded") and state.get("summary"):
                return "CQL 模型已载入，保持暂停；确认游戏后点击继续开始评估"
