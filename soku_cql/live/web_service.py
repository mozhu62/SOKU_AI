...
def create_app(workbench, port):
    # 沿用 PPO 的本机 FastAPI + WebSocket + 命令编号流程，静态资源共享 CQL/PPO 工作台组件。
    app = FastAPI(docs_url=None, redoc_url=None, openapi_url=None)
    host = f"127.0.0.1:{port}"

    @app.middleware("http")
    async def local_only(request: Request, call_next):
        if (request.headers.get("host") != host or not _same_origin(request.headers.get("origin"), host)
    ...
    @app.get("/")
    def index():
        return RedirectResponse("/play.html")
    ...
    def history(kind: str = "rounds", limit: int = 100):
        if kind != "rounds":
            raise HTTPException(400, "实战服务只有小局历史，没有 PPO/CQL 训练损失")
    ...
    async def stream(websocket: WebSocket):
        ...
        if (websocket.headers.get("host") != host or not _same_origin(websocket.headers.get("origin"), host)
                or protocols != ["soku-cql"]):
            await websocket.close(code=1008)
            return
        await websocket.accept(subprotocol="soku-cql")
        try:
            while True:
    ...
    app.mount("/", StaticFiles(directory=WEB_DIST, html=True), name="cql-workbench")
    return app
...
def run_workbench(config, auto_start=False):
    ...
    if not (WEB_DIST / "play.html").is_file():
        raise RuntimeError("浏览器实战页面尚未构建：在 soku_cql 目录手动执行 npm --prefix web run build；不再使用 Tk 界面")
    web = config["web"]
    ...
    try:
        ...
        print(f"CQL 实战工作台：http://127.0.0.1:{port}/play.html", flush=True)
