...
from ..action_space import to_controller, validate_buttons
...
class SafeControl:
    ...
    def __init__(self, config):
        ...
        if error == 183:
            ...
            self.mutex = None
            raise RuntimeError("已有 CQL/PPO 控制器运行，请先关闭；DQN 控制器也不能同时使用")
        ...
        self.thread = threading.Thread(target=self._watch, name="cql-input-watchdog", daemon=True)
        self.thread.start()
