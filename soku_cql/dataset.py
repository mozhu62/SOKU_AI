...
def read_shard(path: Path, positive_down: bool):
    try:
        with np.load(path, allow_pickle=False) as source:
            ...
            shard = {key: source[key] for key in source.files if key != "metadata_json"}
        ...
        expected.update({key: (count,) for key in ("episode_id", "action_horizontal", "action_vertical",
                                                  "action_duration", "transition_valid", "rewards", "terminated")})
        ...
        # 终局帧可以作为 next_state，但不得成为下一段的训练动作起点。
        valid[1:] &= ~terminal[:-1]
