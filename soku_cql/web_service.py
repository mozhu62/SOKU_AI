...
def create_app(runtime, port):
    ...
    @app.get("/api/status")
    def status():
        return {**runtime.snapshot(), "ui_mode": "cql_training"}
    ...
    async def stream(websocket: WebSocket):
        ...
        try:
            while True:
                await websocket.send_json({**runtime.snapshot(), "ui_mode": "cql_training"})
...
def run_workbench(runtime):
    ...
    if not (WEB_DIST / "index.html").is_file():
        raise RuntimeError("CQL 前端尚未构建：在 web 目录执行 npm ci 和 npm run build；或使用 --headless")
    ...
    try:
        ...
        print(f"CQL 离线训练工作台：http://127.0.0.1:{port}/", flush=True)
        print(f"SSH 转发（在你的电脑执行）：ssh -N -o ExitOnForwardFailure=yes -L 127.0.0.1:8776:127.0.0.1:{port} 用户名@服务器地址", flush=True)
        print("转发后本地浏览器：http://localhost:8776/ （无 token、无额外路径）", flush=True)
