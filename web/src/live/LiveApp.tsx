...
export default function LiveApp(){
  ...
  if(state.ui_mode&&state.ui_mode!=='bc_live')return <main className="login-note"><h1>这是离线训练服务</h1><p>请在 Windows 用 scripts/play.py 启动实战，再打开它打印的地址。离线训练模型不会从网页直接接管游戏。</p></main>;
  return <div className="app-shell"><aside className="sidebar"><div className="brand"><span className="brand-mark">S</span><div><strong>SOKU / BC</strong><small>实战工作台</small></div></div><nav aria-label="实战模块">{tabs.map(([id,label,Icon])=><button key={id} className={id===tab?'active':''} onClick={()=>setTab(id)}><Icon size={18}/><span>{label}</span></button>)}</nav><div className="sidebar-foot"><span className="local-dot"/>本机 · {location.host}<small>固定权重 / Joint432 分类</small></div></aside>
