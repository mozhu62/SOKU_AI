...
export function Parameters({state}:{state:Row}){
  ...
  return <><div className="section-intro"><div><h1>参数与记录</h1><p>沿用工作台配置流程：编辑值与运行值分开，暂停后应用并新建会话。</p></div></div>
    <Card title="实战参数" note="不更改训练损失、网络结构或模型权重；分类选择固定为 argmax，不采样"><div className="form-grid">
      <label className="field">推理设备<input value={draft.device??''} onChange={e=>setDraft({...draft,device:e.target.value})} placeholder="cpu / cuda / cuda:0 / hybrid"/><small>hybrid：CPU 编码与输出，TCN 使用 CUDA:0；需要可用 CUDA。</small></label>
      ...
      <label className="field"><span>TCN CUDA Graph</span><input type="checkbox" checked={!!draft.tcn_cuda_graph} onChange={e=>setDraft({...draft,tcn_cuda_graph:e.target.checked})}/><small>仅 GPU 增量模式生效；加载时捕获，失败请关闭后重新加载。运行后端：{state.prediction?.tcn_backend??'尚未推理'}</small></label>
