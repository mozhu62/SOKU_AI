...
export function Parameters({state}:{state:Row}){
  ...
  return <><div className="section-intro"><div><h1>参数与记录</h1><p>沿用工作台配置流程：编辑值与运行值分开，暂停后应用并新建会话。</p></div></div>
    <Card title="实战参数" note="不更改训练奖励、网络结构或模型权重"><div className="form-grid">
      ...
      <label className="field">决策间隔 / 游戏帧<input type="number" min={1} max={60} value={draft.decision_interval_frames??1} onChange={e=>setDraft({...draft,decision_interval_frames:+e.target.value})}/></label>
