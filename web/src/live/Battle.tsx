...
export const buttonNames=['体术 A','DASH D','轻弹幕 B','重弹幕 C','切卡','使用符卡'];
...
export function Battle({state}:{state:Row}){
  ...
  return <><div className="section-intro"><div><h1>BC 战斗与模型输出</h1><p>单帧完整按键 · 确定性 argmax · 概率为 softmax(logits)，不表示胜率或命中率。</p></div></div>
    ...
    <Card title="ABCD 与卡牌按键核对" note={`最近模型观测：小局 ${p.observation_round??'未记录'} / 帧 ${p.observation_frame??'未记录'}；暂停后保留最近输出，不代表仍在按键`}>
    ...
    <Card title="Joint432 原始输出与概率排序" note={`上一帧实际输入 ${p.previous_joint_action_id===432?'START':state.action_catalog?.[p.previous_joint_action_id]??'未记录'}；上一帧方向持续量 ${number(p.previous_action_duration,3)}（clip 到 60 后 /60）`}>
      <label className="inline-controls">展示范围 <select value={limit} onChange={e=>setLimit(Number(e.target.value))}><option value={20}>Top 20</option><option value={50}>Top 50</option><option value={432}>全部 432</option></select></label>
