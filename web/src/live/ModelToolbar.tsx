  ...
  return <section className="model-toolbar" aria-label="模型选择与加载">
    <div className="model-current">
      ...
      <div className="model-current-text">
        <small>{loaded ? `当前已加载模型 · Joint144 / TCN${state.temporal?.context_frames??'未记录'}` : '尚无可运行模型'}</small>
        <strong title={String(currentPath || '')}>{loaded ? filename(currentPath) : '请选择并加载 BC 模型'}</strong>
