...
export function Dataset({state}:{state:Row}){
  ...
  return <><div className="section-intro"><div><h1>数据与覆盖率</h1><p>按整份 REP 分组 8:2，完整重复 NPZ 不跨集合。序列不跨终局或断帧。</p></div></div>
    ...
    <Card title="卡键重合自动清洗" note="同帧切卡 + 用卡：仅清除切卡，保留用卡和其它按钮；不删帧、不修改源 NPZ。动作空间仍为 432 类。">
    ...
    <Card title="固定划分清单" note={state.data?.split_hash?`校验 ${state.data.split_hash.slice(0,16)}`:'未准备'}>{error&&<p className="error">{error}</p>}
      ...
      <DataTable rows={files.rows} columns={[{key:'name',title:'NPZ'},{key:'split',title:'集合',render:v=>v==='train'?'训练':'验证'},{key:'frames',title:'状态帧'},{key:'transitions',title:'有效动作帧'},{key:'card_overlap_cleaned_rows',title:'清洗行',render:v=>v==null?'未记录':number(v,0)}]}/>
    </Card>
  </>;
}
