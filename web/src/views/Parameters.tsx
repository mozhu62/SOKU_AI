...
export function Parameters({state}:{state:Row}){
  ...
  useEffect(()=>{void refresh().catch(e=>setMessage(String(e)));},[state.stage,state.output]);
  ...
  const apply=async()=>{setBusy(true);try{
    ...
    const result=await command('configure',{training:changes,expected_stage:schema.stage,expected_output:schema.output});setMessage(result.message);if(result.status==='completed')await refresh();
  ...
  return <><div className="section-intro"><div><h1>超参数与模型记录</h1><p>暂停后应用；保留优化器状态，每次修改保存快照并标记新阶段。</p></div><Button variant="secondary" disabled={busy} onClick={()=>void refresh().catch(e=>setMessage(String(e)))}>读取当前配置</Button></div>
    ...
    <Card title="模块冻结" note="冻结仅停止权重更新，不等于去除；TCN 模式的 GRU 已旁路，不参与输出。架构切换请用「时序实验」。"><div className="inline-controls"><Button variant="secondary" disabled={busy||locks.includes('frozen_modules')} onClick={()=>setFrozen([])}>全部有效模块训练</Button><Button variant="secondary" disabled={busy||locks.includes('frozen_modules')} onClick={()=>setFrozen(schema.modules.filter((x:string)=>!x.endsWith('_head')))}>仅训练 BC 分类头</Button><Button variant="secondary" disabled={busy||locks.includes('frozen_modules')||schema.temporal?.mode==='tcn'} onClick={()=>setFrozen([...new Set([...frozen,'gru'])])}>冻结 GRU（仍参与计算）</Button></div>
    ...
    <Card title="固定配置与网络结构" note="TCN32 上下文固定为 31 帧历史＋当前帧；跨 GRU/TCN 不能续训。序列长度、AMP、缓存和划分在 YAML 中修改后重启"><pre>{JSON.stringify(schema.config,null,2)}</pre></Card>
