...
export function Parameters({state}:{state:Row}){
  ...
  return <><div className="section-intro"><div><h1>超参数与模型记录</h1><p>暂停后应用；保留优化器状态，每次修改保存快照并标记新阶段。</p></div><Button variant="secondary" disabled={busy} onClick={()=>void refresh().catch(e=>setMessage(String(e)))}>读取当前配置</Button></div>
    ...
    <Card title="模块冻结" note="冻结仅停止权重更新，不会移除该模块的前向影响；当前架构已彻底删除 GRU。"><div className="inline-controls"><Button variant="secondary" disabled={busy||locks.includes('frozen_modules')} onClick={()=>setFrozen([])}>全部模块训练</Button><Button variant="secondary" disabled={busy||locks.includes('frozen_modules')} onClick={()=>setFrozen(schema.modules.filter((x:string)=>!x.endsWith('_head')))}>仅训练 Joint144 分类头</Button></div>
