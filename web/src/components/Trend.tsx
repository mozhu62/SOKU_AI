...
export function Trend({title,rows,series,note}:{title:string;rows:Row[];series:{key:string;name:string;color:string}[];note?:string}){
  ...
  return <Card title={title} note={note}>{rows.length?<div className="chart"><ResponsiveContainer width="100%" height="100%"><LineChart data={data} margin={{top:5,right:16,bottom:0,left:5}}>
    <CartesianGrid stroke="#28364d" strokeDasharray="3 3"/><XAxis dataKey="step" tick={{fill:'#95a5bc'}} minTickGap={40}/><YAxis width={62} tick={{fill:'#95a5bc'}} domain={['auto','auto']}/>
    <Tooltip contentStyle={{background:'#132239',border:'1px solid #405575'}} labelFormatter={value=>`更新步 ${value}`}/><Legend/>
    {series.flatMap(s=>stages.map(stage=><Line key={`${s.key}_${stage}`} dataKey={`${s.key}_${stage}`} name={`${s.name}${stages.length>1?` · 阶段 ${stage}`:''}`} stroke={s.color} strokeWidth={2} dot={rows.length<10?{r:3}:false} isAnimationActive={false} connectNulls/>))}
