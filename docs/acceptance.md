...
原 BC 的动作/schema/资源输入保持不变。时序实验新增 GRU/TCN 分支、冻结规则、独立输出和前端比较。两份对照 YAML 指向本机已存在的相邻 `soku_cql/data/replay_shards_resources_v4_mirror`；基础 YAML 的项目内数据路径需由使用者核对，不能将路径存在等同于全量数据校验通过。运行结果、吞吐率、测试通过与浏览器显示效果未验证。
...
- `tests/test_temporal_experiment.py`：TCN 32 帧因果性、流式/批量对齐、短前导、GRU 旁路和冻结、同种子公共层初始化、版本兼容。
- `tests/test_temporal_runtime.py`：独立实验成功/失败隔离、配置来源校验、请求幂等、实战时序连续性。

时序实验的完整步骤和源码覆盖见 [GRU/TCN32 实验说明](temporal_experiments.md)。
