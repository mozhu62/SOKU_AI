...
from soku_bc.live.config import load_config, validate


def main():
    parser = argparse.ArgumentParser(description="BC 模型 Windows 实战验证；只推理，不训练")
    ...
    parser.add_argument("--port", type=int, help="实战网页起始端口，默认 8797，冲突时自动递增")
    ...
    if not args.headless:
        from soku_bc.live.web_service import WEB_DIST, run_workbench
