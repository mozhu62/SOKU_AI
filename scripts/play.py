...
from soku_cql.live.config import load_config, validate


def main():
    parser = argparse.ArgumentParser(description="CQL 模型 Windows 实战验证；只推理，不训练")
    parser.add_argument("--config", default="configs/live_eval.yaml")
    parser.add_argument("--checkpoint", help="可选初始 CQL .pt；也可在网页顶部直接选择本机模型")
    ...
    parser.add_argument("--port", type=int, help="实战网页起始端口，默认 8777，冲突时自动递增")
    ...
    if not args.headless:
        from soku_cql.live.web_service import WEB_DIST, run_workbench
        ...
        return
    from soku_cql.live.runtime import LiveRuntime
