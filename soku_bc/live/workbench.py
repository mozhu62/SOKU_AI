...
class Workbench:
    ...
    def snapshot(self):
        ...
        state.update(ui_mode="bc_live", workbench_busy=busy, workbench_error=error, runtime_config=config,
    ...
    def _execute(self, name, value):
        ...
        for key in ("device", "rounds", "cpu_threads", "amp", "streaming_tcn"):
            if key in value:
        ...
        while not self.closed.wait(0.1):
            ...
            if state.get("loaded") and state.get("summary"):
                return "BC 模型已载入，保持暂停；确认游戏后点击继续开始评估"
