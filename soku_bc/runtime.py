...
class Runtime:
        ...
        while True:
            ...
            try:
                if name == "resume":
                    ...
                    self.paused = False
                    self.publish(message="正在从固定离线数据进行 BC 更新")
                elif name == "pause":
                    self.paused = True
                    self._close_prefetch()
                elif name == "save":
    ...
    def _validate(self):
        ...
        result.update(batches=len(rows), seconds=time.perf_counter() - started,
                      ...
                      majority_baseline_accuracy=result["joint_data"][majority] / result["samples"],
                      selection_metric="validation_nll_v1", step=self.step, stage=self.stage)
    ...
    def _run(self):
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                self.publish(state="training", message="正在从固定离线数据进行 BC 更新")
